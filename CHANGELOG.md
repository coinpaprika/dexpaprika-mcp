...
- README: the install section claimed "The server runs on port 8010 by default. You'll see `MCP server is running at http://localhost:8010`". It does not. This is a stdio server, it binds no port, and it prints `DexPaprika MCP server v<version> (tool contract v<contract>) is running...` to stderr. The `dexpaprika-mcp --version` verification step did not work either: the flag is ignored and the server just starts.
